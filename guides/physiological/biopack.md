...
# A quick guide to *Biopack*


This is a quick overview of COBE Lab’s *Biopac System* which is used to
...
analysis software AcqKnowledge is then used observe, process and analyse
data. COBE Lab has nine AcqKnowledge licenses.
...
- One GSR-100C: wired Galvanic Skin Response (aka Electro-Dermal
  Activity) unit
...
- Three Senstream Research Rings: wireless rings that gather EDA at 400
  Hz, PPG at 400 Hz, EGA at 800 Hz and temperature.
  [(spec)](https://www.biopac.com/product/research-ring/#product-tabs)

- Various electrodes.

## Pre-Setup

Biopac is the firm that produces and provides the equipment for
measuring physiological signals. For this guide we will look at how we
...
screen. Firstly we will introduce you to the different hardware you’ll
need.
...
The modules are the receivers of the wearable devices that the subjects
wear inorder to collect the physiological signals. There are several of
these that connect to the different wearable devices. Inorder to know
which are connected to which please see …

### acqknowledge

Here at Cobe-lab we also use their [acqknowledge
...
In order to ensure temporal control it will many times be nessecary to
be able to send signals to the software that collects and stores the
...
voltage difference at simulus onest. This essentially gives you another
variable that tells you when signals were sent, which is partiularly
important in event related designs.

## The Setup

In general the setup follows the structure of; \* Setting up the
hardware and their connections. \* Ensuring connection from wearable
devices and acqknowledge. \* Ensuring reliable triggers from the
experimental script to acqknowledge.
...
MP150 sound look something like below:
...
acqknowledge software (here you need one of the acqknowledge keys)

## 

## 

when opened you should nagviate to …. and then …. in order to select the
...
Now you can select start recording inorder to see if a signal goes
through, here it is very convinient to have an easily manipulated signal
